...
export class MealsService {
  ...
  async addMeal(addMeal: AddMeal, userId) {
    ...
    const info = await lastValueFrom(
      this.httpService
        .get(
          `https://api.calorieninjas.com/v1/nutrition?query=${allIngredients}`,
          {
            headers: {
              'X-Api-Key': 'eyjsQXF1lTy9nC1n5DJcmg==NWKXyzOp9BjEsPvI',
            },
          },
        )
  ...
  async createMealPlan(date, userId) {
    ...
    return this.dailyMealPlanRepository.save({
      ...date,
  ...
  async getDailyMealPlan(date, userId) {
    ...
    const [mealPlan] = await this.dailyMealPlanRepository.find({
      date: new Date(date),
      userId,
    });

    return {
      breakfast: await this.mealRepository.findOne({ id: mealPlan.breakfast }),
  ...
  searchByKeyword(keyword: string) {
    Logger.log(`MealsService.searchMealsByKeyword(${keyword})`);
    return this.mealRepository.searchByKeyword(keyword);
  }
}
